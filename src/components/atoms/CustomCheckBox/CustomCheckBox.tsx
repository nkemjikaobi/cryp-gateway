  ...
  return (
    <div className="block">
      <div className={`flex items-center ${className}`}>
        ...
        {isDisabled ? (
          <input
            className={`${size === CheckBoxProperties.SIZES.small ? "w-8 h-8" : "w-8 h-8"}
          ${
...
`}
          ...
          mx-2  cursor-pointer drop-shadow-sm bg-white border border-citiGray-50 hover:border-crypGreen-800 hover:bg-crypGray-200
          text-crypGreen-800 hover:text-crypGreen-800 border-[#EBEFF1] focus:ring-0`}
            type="checkbox"
            {...otherProps}
          />
        )}
